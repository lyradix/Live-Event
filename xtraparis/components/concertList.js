...
import {checkbox,StyleSheet, Switch, View, Text, FlatList, SafeAreaView,Pressable, TouchableOpacity, Button} from 'react-native';
...
const [timesPressed, setTimePressed] = useState(null);
const [toggle, setToggle] = useState(null);


let textLog = '';
if (timesPressed > 0) {textLog = 'onPress'}
...
//filtre pour le jour de Dimanche
const concertFilterSunday = concert.filter(function(item){
    return item.jour == 'Dimanche';
 }).map(function({artist,horaire}){
     return {artist,horaire};
 });
